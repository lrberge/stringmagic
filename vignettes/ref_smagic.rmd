...
output:
  ...
  pdf_document:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{string_magic_walkthrough}
...
Before moving on, we set the global option `.string_magic.class` to `TRUE` such that 
the character vectors produced by `string_magic` are displayed nicely in this vignette.
```{r}
library(stringmagic)
string_magic = string_magic_alias(.class = "string_magic")
```
...
```{r}
name = "john doe"
string_magic("Hi {title ? name}.")

cars = row.names(mtcars)
print(cars)
string_magic("I like cars starting with C like {'i/^c'get, x, unik, enum.or ? cars}.")
...
Finally, `string_magic("--help")` gives you a compact help.
...
Now let's come to what `string_magic` really brings to the table.
...
string_magic("He said: {upper.sentence, Q ? x}")
```
...
mark to trigger verbatim evaluation. For example `string_magic("hello {!x}")` would lead to "hello x". 
...
expression becomes evaluated with the function `string_magic`.
...
```{r}
n = 2
string_magic("poly({n}): {' + 'c ! {letters[1 + 0:n]}x^{0:n}}")
```
...
```{r, eval=FALSE}
string_magic("{'arg1'op1.optionA.optionB, arg2 op2.optionC, 
...
# let's shorten this
string_magic("Oreste: {s, '4|..'k, c, Q ? x}")
...
string_magic("Camille: {s, '4||..'k, c, Q ? rome}")
```
