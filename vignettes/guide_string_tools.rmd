...
string_magic("All cars from mtcars:\n{C, 60 width ? cars}")
...
It has access to all (50+) [`string_magic` operations](LINK), allowing for
...
1. use `"@op1, op2"` to perform any arbitrary sequence of [`string_magic` operation](LINK)
...
The interpolation is performed with [`string_magic`](LINK). This means that any [`string_magic` operation](LINK)
