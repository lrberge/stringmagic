...
It has access to all (50+) [`smagic` operations](LINK), allowing for
...
1. use `"@op1, op2"` to perform any arbitrary sequence of [`smagic` operation](LINK)
...
The interpolation is performed with [`smagic`](LINK). This means that any [`smagic` operation](LINK)
...
Since interpolations are resolved with `smagic`, you can add any text before/after the 
interpolation:
