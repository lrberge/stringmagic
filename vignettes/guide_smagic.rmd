...
output:
  ...
  pdf_document:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{smagic_guide}
...
Almost anything `glue` can do, `smagic` can do.
...
```{r}
smagic("Hi {x}! How's {y} doing?", y = "Jane")
```

So far, so good. Now let's come to `smagic`'s originality: the ability to add operations to interpolations.

# How to add operations to interpolations {#sec_interpol}

In `smagic` you can apply any arbitrary chain of operations to the interpolated variables. 
...
```{r}
smagic("The max of each variable is: {enum ? sapply(iris[, 1:4], max)}.")
```
...
```{r}
smagic("The first two species are: {unik, '2'first, q, enum ? iris$Species}.")
```

### Alternative ways to pass arguments 

There are three ways to pass arguments to operations:
![passing-arguments](images/passing-arguments.png)
...
```{r}
smagic("The first five sepal lengths are: {5 first, enum ? iris$Sepal.Length}.")
```
...
```{r}
n = 5
smagic("The first {N?n} sepal lengths are: {`n`first, enum ? iris$Sepal.Length}.")
```
...
smagic("{upper.sentence ? oversight}")
```

**Wrapping up.** With `smagic` you can perform string interpolations *à la* `glue` and 
also empower these interpolations by adding operations on-the-fly (see the [operations reference](LINK)). 
...
In `smagic`, you have two types of interpolations: regular interpolations (the ones we have covered so far),
and verbatim evaluation. The syntax of vertatim interpolations is as follows:

![verbatim](man/figures/verbatim.png)
...
The main interest of verbatim evaluations is nesting. Indeed, the function `smagic` is always applied 
...
sum from `x1` to `x3`:
```{r}
smagic("y = {' + 'collapse ! x{1:3}}")
```

![example-verbatim](man/figures/example-verbatim.png)
...
```{r}
n = 2
smagic("poly({n}): {' + 'collapse ! {letters[1 + 0:n]}x^{0:n}}")
```
...
[vignette on flags](LINK) for more information.
...
messages to inform the user. `smagic` has an extensive support for pluralization, which we 
briefly introduce here. 

The syntax for pluralization is as follows:

![example-verbatim](man/figures/plural.png)
...
```{r}
nFiles = 6
smagic("Warning: {N?nFiles} file{#s, is} corrupted.")
...
please see the [dedicated section](LINK).
...
**Escape brackets.** The opening and closing brakets, `{}`, are special characters 
...
Ex.2: using regular interpolation to pass verbatim text.
```{r}
smagic("I {'can {write} {{what}} I want'}")
```

**Other escapes.** In the few operations expecting a semi-colon (*if-else* and certain 
operations in *pluralization*), it can also be escaped with a double backslash.

Ex.3: if-else statement with semi-colon.
```{r}
is_c = TRUE
...
**Use a regular `!`.** In interpolations, the exclamation mark (`!`) signals a verbatim expression. 
