...
The function `string_clean` is specialized in cleaning and has a dedicated syntax:
...
`stringr::string_detect` works pretty well. You feed a regex, and done. 
...
For pattern detection, `stringmagic` offers three functions: `string_is`, `string_which` and `string_get`.
...
# in interactive mode, string_get keeps the previous item in memory, 
...
you can use `string_ops`. It has access to all operations available to `smagic`.
