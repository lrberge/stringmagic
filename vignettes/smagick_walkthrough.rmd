...
- [the slash operator](#sec_slash): to quickly create character vectors
...
smagick("{upper, Q! hi}", 
        #       ^                      => adds a space before 'hi'
        "{upper, Q ! hi}", 
        #      ^ ^                     => no space added (ie decorative space)
        "{upper, Q !  hi}", sep = "\n")
        #      ^ ^^                    => adds a space before 'hi'
```
...
Splits the string according to a pattern. The two operations have different defaults: `' '` 
...
To concatenate multiple strings into a single one. The two operations are 
...
Replaces a pattern with a string. The two operations are identical and have no default.
...
Formats integers by adding a comma to separate thousands. Options: `"letter"`, `"upper"`, `"0"`, `"zero"`.
...
numeric vectors with 0s. The upper case command (`N`) adds the option `"letter"`.
```{r}
x = c(5, 12, 52123)
...
To keep only the first `n` characters (like `cfirst` but with more options). The
...
Fills the character strings up to a size. Options: `"right"`, `"center"` and a free-form symbol.
...
maximum size of the character string is used. See help for [`str_fill()`](https://lrberge.github.io/stringmagick/reference/str_fill.html) 
...
# Special interpolation: The slash {#sec_slash}
...
- if the vector contains any *box*, extra interpolations are resolved and appended 
in a vector fashion

In Ex.1 the string `"one, two, {x}"` is taken as verbatim and split w.r.t. commas, 
leading to `c("one", "two", "{x}")`. 
Since the last element contained an opening box, it is interpolated and inserted into the vector, 
leading to the result.

By default, thanks to the argument `slash = TRUE`, you can apply the slash operator without
the need of an interpolation box (provided the slash appears as the first character), 
...
This is useful to quickly create string vectors.
...
is not recognized as one the commands previously described is treated as a verb.
...
Ex.3: using regular interpolation to pass verbatim text.
```{r}
