...
See detailed explanations in [`str_clean()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#sec_clean). 
...
By default it uses the same syntax as [`str_get()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#detect_funs) 
...
Mostly useful as the final operation in a [`str_ops()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#sec_ops) call.
By default it uses the same syntax as [`str_is()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#detect_funs) 
so that you can use regex flags and 
include logical operations between regex patterns with `' & '` and `' | '`.
If the option `"equal"` is used, a simple string equality with the argument is tested (hence
no flags are accepted). If the option `"in"` is used, the argument is first split with respect to commas
...
By default it uses the same syntax as [`str_which()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#detect_funs) 
so that you can use regex flags and 
include logical operations between regex patterns with `' & '` and `' | '`.
If the option `"equal"` is used, a simple string equality with the argument is tested (hence
no flags are accepted). If the option `"in"` is used, the argument is first split with respect to commas
...
the vector using [`str_clean()`](https://lrberge.github.io/stringmagic/articles/string_tools.html#sec_clean). 
...
See help for [`str_fill()`](https://lrberge.github.io/stringmagic/reference/str_fill.html) 
...
use the function `smagic_register`. 
...
Finally, you need to register the function with `smagic_register` and provide it an
...
More generally, the function taken by `smagic_register` is called internally 
...
# In smagic_register, the valid_option argument is used to validate them.
...
smagic_register(keep_varnames, "keepvar")
