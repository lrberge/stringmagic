...
By default the function `string_magic` returns a plain character vector. This may not render
nicely in this vignette. Hence we create an alias to `string_magic` which applies `base::cat`
...
string_magic("Troubles ahead: {', 'split, ~(' loves 'c), C ? x}.")
```
...
string_magic("{' 'split, if(.nchar<=4 ; nuke ; '7|..'k), c ? x}")
```
...
string_magic("pvalues: {vif(.<1e-16 ; <1e-16 ; {%05f?.}), fill.right ? pval}", .collapse = "\n")
```
...
string_magic("{$enum, is, (a;) ? pple} tall guy{$s}.",
