...
This document references all regular `smagic` operations. They can be used within 
`smagic` or can be accessed from `string_ops` or `string_clean`. 
...
see the [dedicated section](https://lrberge.github.io/stringmagic/articles/string_tools.html).
...
so that group-wise operations can be applied. See the [section on group-wise operations](LINK).
```{r}
# 'Split' with its default (comma separation)
smagic("{Split ! romeo, juliet}")
...
# regular way
x = 1:4
smagic("And {' and 'collapse ? x}!")

# with s2
smagic("Choose: {', | or 'collapse ? 2:4}?")

# default of Collapse: enumeration (similar to operation enum)
wines = c("Saint-Estephe", "Margaux")
smagic("I like {Collapse ? wines}.")

# default of collapse: space concatenation
smagic("{split, '.{5,}'get, collapse ! I don't like short words}")
```
...
smagic("Endings with i: {'i\\w*'extract, ~(', 'collapse), enum.1 ? x}.")
...
smagic("Mercedes models: {'Merc & [[:alpha:]]$'get, '^.+ 'r, enum ? x}.")

models = c("Merc 230", "Merc 450SE", "Merc 480")
# we only ekep the ones in the set
smagic("Mercedes models: {`models`get.in, enum ? x}.")
```
...
```{r}
smagic("First 3 mpg values: {3 first, enum ? mtcars$mpg}.")

# you could have done the same with regular R in the expression...
smagic("First 3 mpg values: {enum ? head(mtcars$mpg, 3)}.")

# ...but not in the middle of an operations chain
smagic("First 3 integer mpg values: {'f/!.'get, 3 first, enum ? mtcars$mpg}.")
```
...
```{r}
smagic("Letters in the middle: {13 first, 5 last, enum ? letters}.")

smagic("First and last letters: {'3|3'first, enum ? letters}.")

smagic("Last letters: {-21 first, enum ? letters}.")
```
...
```{r}
# basic use
smagic("First 3 letters: {'3'K, q, enum ? letters}.")

# advanced use: using the extra argument
smagic("The letters are: {q, '3|:rest: others'K, enum ? letters}.")
```
...
last `n` values.
```{r}
smagic("Last 3 mpg values: {3 last, enum ? mtcars$mpg}.")

smagic("Removing the 3 last elements leads to {-3 last, enum ! x{1:5}}.")
```
...
smagic("{', | and 'split, '\\D'sort, enum ? x}")

# we extract the numbers, then convert to numeric, then sort
smagic("{', | and 'split, '\\D'sort.num, enum ? x}")
```
...
```{r}
smagic("Iris species: {unik, upper.first, enum ? iris$Species}.")
```
...
smagic("Small numbers only: {if(.>20 ; nuke), enum ? x}.")
```
...
```{r}
smagic("{'3'insert.right, ' + 'collapse ? 1:2}")
```
...
```{r}
x = c(5, 12, 52123)
smagic("She owes {n, '$'paste, enum ? x}.")
...
# roman
smagic("What's nicer? {collapse?11:13}, {n.roman, c?11:13} or {n.Roman, c?11:13}?")
```
...
```{r}
smagic("They lost {enum ! {ntimes ? c(1, 12)} against {S!Real, Barcelona}}.")
```
...
smagic("{'[ ,]+'split, upper.first, fill.center, q, '\n'collapse ? life}")

# fixing the length and filling with 0s
smagic("{'5|0'fill.right, enum ? c(1, 55)}")
```
...
```{r}
smagic("6 = {'|'paste.both, ' + 'collapse ? -3:-1}")
```
...
## Formatted time difference: dtime
