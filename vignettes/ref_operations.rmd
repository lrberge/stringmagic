...
By default the function `string_magic` returns a plain character vector. To have a nicer
display within this vignette we create an alias to `string_magic` which applies `base::cat`
just before leaving the function.
```{r}
library(stringmagic)
string_magic = string_magic_alias(.post = base::cat)
```
...
```{r}
x = c(1, 12345) 
string_magic("left:   {format, q, enum ? x}", 
...
string_magic("{'[ ,]+'split, upper.first, fill.center, q, '\n'collapse ? life}")
...
```{r}
string_magic("6 = {'|'paste.both, ' + 'collapse ? -3:-1}")
```
...
```{r}
string_magic("The length of 1:5000:", 
       " - len     = {len ? 1:5000}", 
